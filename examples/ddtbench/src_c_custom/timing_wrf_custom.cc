...
struct pack_info_t {
  ...
  size_t buf_size;
  int ie, is, je, js, ke, ks, number_2D, number_3D, number_4D;
...
coro::generator<MPI_Count> pack_unpack_coro(pack_info_t *info)
{
  ...
  int *array2Ds = info->array2Ds;
  int *array3Ds = info->array3Ds;
  int *array4Ds = info->array4Ds;
  int counter = 0;
  ...
  if (info->pack_unpack == 0) {
    ...
    for(int m=0 ; m<number_4D ; m++ ) {
      for(int k=param_first_scalar ; k<limit_4D_arrays[m] ; k++) {
        for(int l=js ; l<=je ; l++ ) {
          for(int n=ks ; n<=ke ; n++ ) {
            for(int o=is ; o<=ie ; o++ ) {
...
int query_cb(void *state, const void *buf, MPI_Count count, MPI_Count *packed_size) {
  ...
  size_t count = (ilen*jlen*number_2D) + (ilen*jlen*klen*number_3D);
  for(int m=0; m<info->number_4D; m++) {
    for(int k=info->param_first_scalar; k<info->limit_4D_arrays[m]; k++) {
      count += (ilen*jlen*klen);
    }
  }
  *packed_size = sizeof(double) * count;
  return MPI_SUCCESS;
}
  ...
  MPI_Count *used)
{
  ...
  info->buffer = dst;
  if (info->coro.next()) {
    *used = info->coro.getValue().get();
    return MPI_SUCCESS;
  } else {
    assert(!"Pack called without data left!");
    return MPI_ERROR;
  }
}
  ...
  MPI_Count src_size)
{
  ...
  info->buffer = (void*)src;
  if (info->coro.next()) {
    assert(!"Unpack called without data left!");
    info->coro.getValue().get();
    return MPI_SUCCESS;
  } else {
    return MPI_ERROR;
  }
